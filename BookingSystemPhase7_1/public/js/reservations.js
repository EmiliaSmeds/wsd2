...
function createUserIdInput(container) {
  const input = document.createElement("input");
...
function createStartTimeInput(container) {
  const input = document.createElement("input");
  ...
  input.type = "date";
  input.placeholder = "e.g., 1.5.2026";
...
function isStartTimeValid(value) {
  const trimmed = value.trim();
...
function createEndTimeInput(container) {
  const input = document.createElement("input");
...
function createNoteInput(container) {
  const input = document.createElement("input");
  ...
  input.name = "note";
  input.type = "text";
  input.placeholder = "e.g., wish for a room with a tv";
...
function createStatusInput(container) {
  const input = document.createElement("input");
...
function attachUserIdValidation(input) {
  const update = () => {
    const raw = input.value;
    if (raw.trim() === "") {
      setInputVisualState(input, "neutral");
    ...
    userIdValid = isUserIdValid(raw);
    setInputVisualState(input, userIdValid ? "valid" : "invalid");
...
function refreshPrimaryButtonState() {
  const valid = resourceNameValid && userIdValid;
  if (formMode === "create") {
...
function clearResourceForm() {
  ...
  userIdArea.value = "";
  userIdArea.dispatchEvent(new Event("input", { bubbles: true }));
...
function selectResource(resource) {
  ...
  resourceNameInput.dispatchEvent(new Event("input", { bubbles: true }));
  userIdArea.value = resource.description ?? "";
  userIdArea.dispatchEvent(new Event("input", { bubbles: true }));
...
function setFormReadOnly(isReadOnly) {
  ...
  if (userIdArea) {
    userIdArea.readOnly = isReadOnly;
  }
...
const userIdInput = createUserIdInput(userIdCnt);
const startTimeInput = createStartTimeInput(startTimeCnt);
const endTimeInput = createEndTimeInput(endTimeCnt);
const noteInput = createNoteInput(noteCnt);
const statusInput = createStatusInput(statusCnt);
attachResourceNameValidatiUserIdArea(userIdCnt);
attachUserIdValidation(userIdArea);
setFormReadOnly(role === "reserver");
