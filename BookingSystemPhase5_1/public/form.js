...
function buildValidationMessage(errors) {
  ...
  return `Unsuccessful request. Please correct the input fields:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg =
          body?.details ||
          "A resource with the name ${resourceName} already exists. Please choose another name.";
        showFormMessage("info", `Please choose another resource name:\n\n${msg}`);
        return;
      }
    ...
    const msgLines = [];
    msgLines.push(`Resource booked successfully!`);
