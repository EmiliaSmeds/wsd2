...
const resourceValidators = [
  ...
  body('resourceName')
    ...
    .matches(/^(?!.*[<>]).*$/).withMessage('Unvalid characters in resourceName')
  ...
  body('resourceDescription')
    ...
    .matches(/^(?!.*[<>]).*$/).withMessage('Unvalid characters in resourceDescription')
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(400).json({
      ok: false, errors: errors.array().map(e => ({ field: e.path, msg: e.msg })), }); }
