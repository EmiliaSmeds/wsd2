...
function createResourceDescriptionArea(container) {
  ...
  container.insertBefore(textarea, helperText);
  return textarea;
}
...
function isFormValid() {
  return (
    isResourceNameValid($("resourceName")?.value ?? "") &&
    isResourceDescriptionValid($("resourceDescription")?.value ?? "") &&
    isResourcePriceValid($("resourcePrice")?.value ?? "")
  );
}
...
const container = document.getElementById("resourceDescriptionContainer");
const resourceDescriptionArea = createResourceDescriptionArea(container);
